...
WWW=		https://github.com/clawdbot/clawdbot
...
do-install:
	@${ECHO_MSG} "===> Installing clawdbot via npm"
	@cd ${WRKSRC} && ${SETENV} HOME=${WRKDIR} npm install -g clawdbot \
		--prefix=${STAGEDIR}${PREFIX}
	@${ECHO_MSG} "===> Building native modules for FreeBSD"
	@cd ${STAGEDIR}${PREFIX}/lib/node_modules/clawdbot && \
		${SETENV} HOME=${WRKDIR} npm install node-addon-api node-gyp --save-dev
	@cd ${STAGEDIR}${PREFIX}/lib/node_modules/clawdbot/node_modules/@mariozechner/clipboard && \
	...
	@${INSTALL_SCRIPT} ${FILESDIR}/clawdbot.in ${STAGEDIR}${PREFIX}/etc/rc.d/clawdbot

post-install:
	@${STRIP_CMD} ${STAGEDIR}${PREFIX}/lib/node_modules/clawdbot/node_modules/@mariozechner/clipboard/*.node 2>/dev/null || true
